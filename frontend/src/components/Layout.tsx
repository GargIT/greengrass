...
const menuItems = [
  { text: 'Översikt', icon: <DashboardIcon />, path: '/', roles: ['ADMIN', 'MEMBER'] },
  { text: 'Hushåll', icon: <HouseholdIcon />, path: '/households', roles: ['ADMIN'] },
  { text: 'Tjänster', icon: <UtilityIcon />, path: '/utility-services', roles: ['ADMIN'] },
  { text: 'Mätaravläsningar', icon: <MeterIcon />, path: '/meter-readings', roles: ['ADMIN', 'MEMBER'] },
  { text: 'Fakturering', icon: <BillingIcon />, path: '/billing', roles: ['ADMIN', 'MEMBER'] },
  { text: 'Rapporter', icon: <ReportsIcon />, path: '/reports', roles: ['ADMIN', 'MEMBER'] },
];

const Layout: React.FC<LayoutProps> = ({ children, onLogout, fullWidth = false }) => {
    ...
    <Box sx={{ display: 'flex' }}>
      ...
      {!isMobile && (
        <IconButton
          ...
          sx={{
            position: 'fixed',
            left: drawerCollapsed ? collapsedDrawerWidth - 16 : drawerWidth - 16,
      ...
      <Box
        ...
        sx={{
          ...
          width: { sm: `calc(100vw - ${drawerCollapsed ? collapsedDrawerWidth : drawerWidth}px)` },
          minHeight: '100vh',
