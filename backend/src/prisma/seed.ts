...
async function main() {
  ...
  for (let i = 1; i <= 14; i++) {
    const household = await prisma.household.upsert({
      ...
      create: {
        ...
        email: `household${i}@grongraset.se`,
        phone: `070-123-${i.toString().padStart(4, "0")}`,
        address: `Gröngräset ${i}`,
        // andelstal: 1/14, // Removed - using equal shares for all households
        // annualMemberFee: 3000, // Removed - using quarterly billing
      },
    });
  ...
  console.log("   Member: member@grongraset.se / member123");
